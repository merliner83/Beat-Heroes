...
export default function HomePage() {
  ...
  const handleBackup = async () => {
    ...
    try {
      const data: any = {};
      const cols = ['studios', 'tracks', 'games', 'learnApps', 'levels', 'patterns', 'learnSubCats', 'articles', 'users'];
      ...
      const a = document.createElement('a'); a.href = url; a.download = `beathero_backup_${new Date().toISOString()}.json`; a.click();
  ...
  const setupStudios = async () => {
    ...
    const sync = async (col: string, id: string, data: any) => {
      ...
      if (!snap.exists()) { await setDoc(ref, data); created++; }
      else {
        const fData = snap.data();
        const payload: any = {};
        let needs = false;
        Object.keys(data).forEach(k => { if (!fData[k]) { payload[k] = data[k]; needs = true; } });
        if (needs) { await setDoc(ref, payload, { merge: true }); fixed++; }
      }
    };
    ...
    try {
      ...
      const arts = [
        ...
        { id: 'art-producing-basics', categoryId: 'intro', title: 'Producing Basics', content: 'Hier lernst du die Grundlagen der Musikproduktion.', minRole: 'free', quiz: [{ question: 'Was ist eine DAW?', options: ['Digital Audio Workstation', 'Dark Audio Wave', 'Dynamic Audio Web'], correctOption: 0 }] },
