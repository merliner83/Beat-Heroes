...
export default function AdminPage() {
  ...
  return <main className="min-h-screen bg-[#050505] text-white p-6 md:p-10"><div className="max-w-6xl mx-auto"><Link href="/"><Button variant="ghost" className="mb-8"><ArrowLeft className="w-4 h-4 mr-2" /> Back</Button></Link><div className="flex items-center gap-4 mb-10"><ShieldCheck className="w-10 h-10 text-primary" /><div><p className="text-xs uppercase tracking-[0.3em] text-primary font-bold">BeatHero Admin</p><h1 className="text-3xl md:text-5xl font-bold tracking-tight">Learn CMS</h1></div></div><section className="space-y-3">{isLoadingArticles ? <Loader2 className="w-6 h-6 animate-spin text-primary" /> : articles?.map(article => <Link key={article.id} href={`/admin/articles/${article.id}`} className="block border border-white/10 rounded-2xl p-5 bg-white/[0.02] hover:bg-white/[0.05] transition-colors"><h2 className="font-bold text-lg">{article.title}</h2><p className="text-sm text-white/40">{article.id} · {article.categoryId} · {article.youtubeUrls?.length || 0} YouTube</p></Link>)}</section></div></main>;
}
