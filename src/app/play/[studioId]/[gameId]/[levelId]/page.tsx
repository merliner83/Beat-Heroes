  ...
  if (!game || !level) {
    return (
      <div className="h-screen bg-[#050505] flex flex-col items-center justify-center text-white p-6 text-center">
        ...
        <h2 className="text-3xl font-black uppercase italic tracking-tighter mb-2">Game Not Found</h2>
        <p className="text-sm opacity-50 mb-8 max-w-xs">The requested level data could not be retrieved from the database.</p>
        <Link href="/">
    ...
    <div className="h-screen bg-[#050505]">
      ...
      {(!sounds || !patterns) && (
        <div className="h-full flex flex-col items-center justify-center text-white gap-4">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-40">Synchronizing Patterns...</p>
        </div>
      )}
    </div>
  );
